...
JSGantt.TaskItem = function(pID, pName, pStart, pEnd, pColor, pLink, pMile, pRes, pComp, pGroup, pParent, pOpen, pDepend, pCaption)
{
      ...
      this.getResource = function(){ if(vRes !== 'Non') return vRes; else return '';  };
...
JSGantt.GanttChart =  function(pGanttVar, pDiv, pFormat)
{
   ...
   this.Draw = function()
   {
      ...
      if(vTaskList.length > 0)
      {
         ...
         vMainTable =
            '<TABLE id=theTable style="margin-left: auto; margin-right: auto" cellSpacing=0 cellPadding=0 border=0><TBODY><TR>' +
            '<TD vAlign=top bgColor=#ffffff style="margin-left: auto; margin-right: auto">';
         ...
         vLeftTable =
            '<DIV class=scroll style="margin-left: auto; margin-right: auto" id=leftside style="width:' + vLeftWidth + 'px"><TABLE cellSpacing=0 cellPadding=0 border=0><TBODY>' +
         ...
         if(vShowRes ==1) vLeftTable += '  <TD style="BORDER-TOP: #efefef 1px solid; FONT-SIZE: 12px; BORDER-LEFT: #efefef 1px solid; WIDTH: 0px; HEIGHT: 20px" align=center nowrap>Resource</TD>' ;
         if(vShowDur ==1) vLeftTable += '  <TD style="BORDER-TOP: #efefef 1px solid; FONT-SIZE: 12px; BORDER-LEFT: #efefef 1px solid; WIDTH: 0px; HEIGHT: 20px" align=center nowrap>Duration</TD>' ;
         if(vShowComp==1) vLeftTable += '  <TD style="BORDER-TOP: #efefef 1px solid; FONT-SIZE: 12px; BORDER-LEFT: #efefef 1px solid; WIDTH: 0px; HEIGHT: 20px" align=center nowrap>% Comp.</TD>' ;
         if(vShowStartDate==1) vLeftTable += '  <TD style="BORDER-TOP: #efefef 1px solid; FONT-SIZE: 12px; BORDER-LEFT: #efefef 1px solid; WIDTH: 120px; HEIGHT: 20px" align=center nowrap>Дата начала</TD>' ;
         if(vShowEndDate==1) vLeftTable += '  <TD style="BORDER-TOP: #efefef 1px solid; FONT-SIZE: 12px; BORDER-LEFT: #efefef 1px solid; WIDTH: 120px; HEIGHT: 20px" align=center nowrap>Дата закрытия</TD>' ;
         ...
         vLeftTable += '</TR>';
            ...
            for(i = 0; i < vTaskList.length; i++)
            {
               ...
               vID = vTaskList[i].getID();
                 ...
                 vLeftTable += '<TR id=child_' + vID + ' bgcolor=#' + vBGColor + ' onMouseover=g.mouseOver(this,' + vID + ',"left","' + vRowType + '") onMouseout=g.mouseOut(this,' + vID + ',"left","' + vRowType + '")>' ;
                  ...
                  '  <TD class=gname style="WIDTH: ' + vNameWidth + 'px; HEIGHT: 20px; BORDER-TOP: #efefef 1px solid; FONT-SIZE: 14px;" nowrap><NOBR><span style="color: #aaaaaa">';
               ...
               if( vTaskList[i].getGroup()) {
                  if( vTaskList[i].getOpen() == 1)
                     vLeftTable += '<SPAN id="group_' + vID + '" style="color:#144d49; cursor:pointer; font-weight:bold; FONT-SIZE: 16px;" onclick="JSGantt.folder(' + vID + ','+vGanttVar+');'+vGanttVar+'.DrawDependencies();">-</span><span style="color:#000000">&nbsp</SPAN>' ;
                  else
                     vLeftTable += '<SPAN id="group_' + vID + '" style="color:#144d49; cursor:pointer; font-weight:bold; FONT-SIZE: 16px;" onclick="JSGantt.folder(' + vID + ','+vGanttVar+');'+vGanttVar+'.DrawDependencies();">+</span><span style="color:#000000">&nbsp</SPAN>' ;
            ...
            '<DIV class=scroll2 id=rightside style="margin-left: auto; margin-right: auto;margin: 0;width: 850px;" >' +
...

         {
	       ...
	       else if (vFormat=='minute')
	       {
            ...
            if( vTaskList[i].getMile()) {
               ...
               vRightTable += '<DIV><TABLE style="margin-left: auto; margin-right: auto; position:relative; top:0px; width: ' + vChartWidth + 'px;" cellSpacing=0 cellPadding=0 border=0>' +
