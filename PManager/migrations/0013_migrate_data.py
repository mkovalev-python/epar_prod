...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        for ms in orm.PM_Milestone.objects.filter(closed=True):
            date_close = ms.date
            ms.statuses.create(
                author_id=1,
        ...
        for ms in orm.PM_Milestone.objects.exclude(closed=True):
            date_create = ms.date_create
            ms.statuses.create(
                author_id=1,
                status=started_status,
                start_date=date_create
            )
            for t in ms.tasks.all():
                ms.manager_id = t.author.id
                break
            ms.save()
    ...
    models = {
        ...
        'PManager.pm_user': {
            ...
            'avatar_color': ('django.db.models.fields.CharField', [], {'default': "'#bcee68'", 'max_length': '20', 'null': 'True', 'blank': 'True'}),
