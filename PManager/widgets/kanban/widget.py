...
from PManager.viewsExt.tasks import taskListXls1
...
def widget(request, headerValues, widgetParams={}, qArgs=[]):
    ...
    """Сортировка задач в последовательности дерева"""
    from PManager.viewsExt.tasks import taskListXls
    aa = taskListXls1(request,True)

    """цикл расстановки id под группировки"""

    tasks_f = {}
    el = 0
    i = 1
    while el < aa.__len__():
        if not aa[el].parentTask_id:
            id = aa[el].id
            parent_id = 0
        else:
            if aa[el].parentTask_id == aa[el-1].parentTask_id:
                i += 1
            else:
                i = 1
            id = str(aa[el].id) + str(i)
            parent_id = tasks_f[aa[el].parentTask_id]['pId']

        try:
            if aa[el].id == aa[el+1].parentTask_id:
                group = 1
            else:
                group = 0
        except:
        ...
        taskGantt = {
            'pId': int(id),
            'pName': aa[el].name,
            'pStart': str(aa[el].dateCreate.month) + '/' + str(aa[el].dateCreate.day) + '/' + str(aa[el].dateCreate.year),
            'pEnd': str(aa[el].deadline.month) + '/' + str(aa[el].deadline.day) + '/' + str(aa[el].deadline.year) if aa[el].deadline else str(aa[el].dateCreate.month) + '/' + str(aa[el].dateCreate.day) + '/' + str(aa[el].dateCreate.year),
            'pColor': 'FF0000' if aa[el].critically > 0.5 else '808080',
            'pLink': '/task_detail/?number=' + str(aa[el].number) + '&project=' + str(aa[el].project_id),
            'pMile': aa[el].critically,
            'pRes': aa[el].resp.first_name + ' ' + aa[el].resp.last_name if aa[el].resp else 'Non',
            'pComp': 0,
            'pGroup': group,
            'pParent': parent_id,
            'pOpen': 0,
            'pDepend': '',
            'pCaption': '',
            'dateCreateDict': datetime.datetime.now()
        }
        tasks_f[aa[el].id] = taskGantt
        el += 1
    listTask =[]
    for t in tasks_f:
        listTask.append(tasks_f[t])
    listTask = sorted(listTask,key=lambda k: k['dateCreateDict'])
