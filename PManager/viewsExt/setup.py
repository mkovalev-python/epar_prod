...
def register(request):
    ...
    if email:
        if not EmailMessage.validate_email(email):
        ...
        if not User.objects.filter(email=email).count():
            ...
            #     u'Ознакомиться с сервисом контроля удаленной работы Экспертная компания',
            #     user,
            #     None,
            #     None,
            #     project=project
            # )
            # task.resp = user
            # task.systemMessage(u'Позиция создана', user, 'TASK_CREATE')
